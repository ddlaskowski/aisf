...
import {
  createControlledRuntimeArchitecturePreview,
  type ControlledRuntimeArchitecturePreview
} from "./governance/controlledRuntimeArchitecturePreview.js";
import {
...
} from "./governance/controlledRuntimeStateModelPreview.js";
import {
...
function buildControlledProjectGenerationContractAuditPreview(): ControlledProjectGenerationContractAudit {
  return createControlledProjectGenerationContractAudit({
    title: "Controlled Project Generation Contract Audit",
    metadata: {
      version: "v12.8",
      source: "controlled-project-generation-contract-audit-cli-preview",
      command: "governance controlled-project-generation-contract-audit",
      readonly: true,
      previewOnly: true
    }
  });
...
function buildControlledRuntimeArchitecturePreview(): ControlledRuntimeArchitecturePreview {
  return createControlledRuntimeArchitecturePreview({
    title: "Controlled Runtime Architecture Preview",
    metadata: {
      version: "v13.0",
      source: "controlled-runtime-architecture-cli-preview",
      command: "governance controlled-runtime-architecture",
      readonly: true,
      previewOnly: true
    }
  });
...
function buildControlledRuntimeComponentContractPreview(): ControlledRuntimeComponentContract {
  return createControlledRuntimeComponentContract({
    title: "Controlled Runtime Component Contract",
    metadata: {
      version: "v13.1",
      source: "controlled-runtime-components-cli-preview",
      command: "governance controlled-runtime-components",
      readonly: true,
      previewOnly: true
    }
  });
...
function buildControlledRuntimeStateModelPreview(): ControlledRuntimeStateModelPreview {
  return createControlledRuntimeStateModelPreview({
    title: "Controlled Runtime State Model Preview",
    metadata: {
      version: "v13.3",
      source: "controlled-runtime-state-model-cli-preview",
      command: "governance controlled-runtime-state-model",
      readonly: true,
      previewOnly: true
    }
  });
...
function buildControlledRuntimeEventModelPreview(): ControlledRuntimeEventModelPreview {
  return createControlledRuntimeEventModelPreview({
    title: "Controlled Runtime Event Model Preview",
    metadata: {
      version: "v13.4",
      source: "controlled-runtime-event-model-cli-preview",
      command: "governance controlled-runtime-event-model",
      readonly: true,
      previewOnly: true
    }
  });
...
function handleCliHelpAndGovernanceUx(argv: string[]): void {
  ...
  if (command === "governance" && args[1] === "controlled-project-generation-contract-audit") {
    ...
    for (const arg of args.slice(2)) {
      ...
      const flag = arg.includes("=") ? arg.slice(0, arg.indexOf("=")) : arg;
      if (!allowed.has(flag)) {
        printAndExit(renderInvalidFlagError("governance controlled-project-generation-contract-audit", flag), 1);
      }
    }

    if (args.includes("--help") || args.includes("-h")) {
      printAndExit(renderGovernanceControlledProjectGenerationContractAuditHelp(), 0);
    }
  ...
  if (command === "governance" && args[1] === "controlled-runtime-architecture") {
    const allowed = new Set(["--json", "--help", "-h"]);
    for (const arg of args.slice(2)) {
      if (!arg.startsWith("-")) {
        continue;
      ...
      const flag = arg.includes("=") ? arg.slice(0, arg.indexOf("=")) : arg;
      if (!allowed.has(flag)) {
        printAndExit(renderInvalidFlagError("governance controlled-runtime-architecture", flag), 1);
      }
    }

    if (args.includes("--help") || args.includes("-h")) {
      printAndExit(renderGovernanceControlledRuntimeArchitectureHelp(), 0);
    }

    const preview = buildControlledRuntimeArchitecturePreview();
    if (args.includes("--json")) {
      printAndExit(JSON.stringify(preview, null, 2), 0);
    }
    printAndExit(renderCliControlledRuntimeArchitecturePreview(preview), 0);
  }

  if (command === "governance" && args[1] === "controlled-runtime-components") {
    const allowed = new Set(["--json", "--help", "-h"]);
    for (const arg of args.slice(2)) {
      if (!arg.startsWith("-")) {
        continue;
      ...
      const flag = arg.includes("=") ? arg.slice(0, arg.indexOf("=")) : arg;
      if (!allowed.has(flag)) {
        printAndExit(renderInvalidFlagError("governance controlled-runtime-components", flag), 1);
      }
    }

    if (args.includes("--help") || args.includes("-h")) {
      printAndExit(renderGovernanceControlledRuntimeComponentsHelp(), 0);
    }

    const contract = buildControlledRuntimeComponentContractPreview();
    if (args.includes("--json")) {
      printAndExit(JSON.stringify(contract, null, 2), 0);
    }
  ...
  if (command === "governance" && args[1] === "controlled-runtime-state-model") {
    const allowed = new Set(["--json", "--help", "-h"]);
    for (const arg of args.slice(2)) {
      if (!arg.startsWith("-")) {
        continue;
      ...
      const flag = arg.includes("=") ? arg.slice(0, arg.indexOf("=")) : arg;
      if (!allowed.has(flag)) {
        printAndExit(renderInvalidFlagError("governance controlled-runtime-state-model", flag), 1);
      }
    }

    if (args.includes("--help") || args.includes("-h")) {
      printAndExit(renderGovernanceControlledRuntimeStateModelHelp(), 0);
    }

    const preview = buildControlledRuntimeStateModelPreview();
    if (args.includes("--json")) {
      printAndExit(JSON.stringify(preview, null, 2), 0);
    }
    printAndExit(renderCliControlledRuntimeStateModelPreview(preview), 0);
  }

  if (command === "governance" && args[1] === "controlled-runtime-event-model") {
    const allowed = new Set(["--json", "--help", "-h"]);
    for (const arg of args.slice(2)) {
      if (!arg.startsWith("-")) {
        continue;
      ...
      const flag = arg.includes("=") ? arg.slice(0, arg.indexOf("=")) : arg;
      if (!allowed.has(flag)) {
        printAndExit(renderInvalidFlagError("governance controlled-runtime-event-model", flag), 1);
      }
    }

    if (args.includes("--help") || args.includes("-h")) {
      printAndExit(renderGovernanceControlledRuntimeEventModelHelp(), 0);
    }

    const preview = buildControlledRuntimeEventModelPreview();
    if (args.includes("--json")) {
      printAndExit(JSON.stringify(preview, null, 2), 0);
    }
